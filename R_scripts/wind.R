...
automate_copernicus_retrieval <- function(start_datetime, end_datetime) {
  ...
  output_filename <- paste0("www/data/", "wind_", format(end_datetime, "%Y-%m-%d"), ".nc")
...
cm.subset(
    ...
    variables=[\"model_wind_to_dir\", \"se_model_speed\"],
...
wind_speed_raster <- raster(nc_file, varname = "se_model_speed")
# Close the .nc file connection
nc_close(nc_data)

wind_speed_rast = terra::rast(wind_speed_raster)
# Load the Tunisian EEZ shapefile as an sf object
tunisian_eez <- st_read("www/eez/eez.shp")

wind_speed_final = process_oceanographic_raster(wind_speed_rast, tunisian_eez)

# Define a color palette for Celsius temperatures
pal <- colorNumeric(palette = "viridis", domain = values(wind_speed_final), na.color = "transparent")

# Create the leaflet map and add the final masked raster
m <- leaflet() %>%
  addTiles() %>%
  addRasterImage(wind_speed_final, colors = pal, opacity = 0.8) %>%
  ...
  addLegend(pal = pal, values = values(wind_speed_final), title = "Wind Speed (m/s)") %>%
